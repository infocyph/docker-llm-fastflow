...
flm_version="$(docker run --rm --entrypoint /opt/fastflowlm/flm "$image" version | head -n1)"
grep -Fq "FLM v${expected_flm}" <<<"$flm_version"

wrapper_version="$(docker run --rm --entrypoint llm-fastflow "$image" version | head -n1)"
